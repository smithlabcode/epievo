...
////////////////////////////////////////////////////////////////////////////////
static void
propose_h_interval(const vector<Path> &paths, const size_t site_id,
  ...
  // scan and copy path
  proposed_path = Path(paths[site_id].init_state, paths[site_id].tot_time);
  size_t start_state = paths[site_id].init_state;
  ...
  while (end_time < time_to_update && seg_id < seg_info.size() - 1) {
    ...
    while (start_jump < paths[site_id].jumps.size() &&
           paths[site_id].jumps[start_jump] < end_time) {
      proposed_path.jumps.push_back(paths[site_id].jumps[start_jump]);
  ...
  while (end_jump < paths[site_id].jumps.size() &&
         paths[site_id].jumps[end_jump] < end_time)
    end_jump++;
  if ((end_jump - start_jump) % 2 == 1)
    end_state = complement_state(end_state);
  ...
  orig_proposal =
  end_cond_sample_prob(ctmm, paths[site_id].jumps,
  ...
  while (end_jump < paths[site_id].jumps.size())
    proposed_path.jumps.push_back(paths[site_id].jumps[end_jump++]);
  
  assert(proposed_path.init_state == paths[site_id].init_state);
  assert(proposed_path.end_state() == paths[site_id].end_state());
}


/* compute acceptance rate */
double
log_accept_rate_interval(const EpiEvoModel &mod, const size_t site_id,
...
bool
Metropolis_Hastings_interval(const EpiEvoModel &the_model, const size_t site_id,
                             vector<Path> &paths,
                             std::mt19937 &gen) {
  vector<SegmentInfo> seg_info;
  collect_segment_info(the_model.triplet_rates,
  ...
  const double u = unif(gen) ;
  const double log_acc_rate =
  log_accept_rate_interval(the_model, site_id, paths, proposed_path,
