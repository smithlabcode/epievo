...
int main(int argc, const char **argv) {
  try {
    ...
    double jump_counts_difference = 0;
    for (size_t i = 0; i < n_paths_to_sample; ++i) {
      ...
      if (leaf_seq.seq == target_leaf_seq.seq) {
        ...
        const size_t J_forward = path_by_site[the_site].jumps.size();
        
        vector<vector<Path> > paths(2, path_by_site);
        vector<Path> proposed_path;
        Gibbs_site(the_model, th, the_site, paths, gen, proposed_path);
        //Gibbs_independent_site(the_model,
        //                       th, the_site, paths, gen, proposed_path);
        const size_t J_Gibbs = proposed_path[1].jumps.size();
        jump_counts_difference += (static_cast<double>(J_forward) - J_Gibbs);
        for (size_t i = 1; i < proposed_path[1].jumps.size(); ++i)
          cerr << proposed_path[1].jumps[i] - proposed_path[1].jumps[i-1 ] << ", ";
        cerr << endl;
      }
    }
    cerr << "Average difference between jump counts: "
         << jump_counts_difference / target_count << endl;
