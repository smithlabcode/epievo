...
////////////////////////////////////////////////////////////////////////////////
///////////////   SITE-INDEPENDENT DOWNWARD PROCESS   //////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
void
expected_sufficient_statistics(const vector<double> rates,
                               const vector<double> init_pi,
                               const vector<>
                               const TreeHelper &th,
                               const size_t site_id,
  ...
  /* get rates and lengths each interval [seg_info: node x segs] */
  vector<vector<SegmentInfo> > seg_info(th.n_nodes, vector<SegmentInfo> (1));
  for (size_t node_id = 1; node_id < th.n_nodes; ++node_id)
    seg_info[i] = SegmentInfo(rates[0], rates[1],
                              paths[node_id][site_id].tot_time);

  /* pruning upward pass [fh: one for each node] */
  vector<FelsHelper> fh(th.n_nodes);
  pruning(th, site_id, paths, seg_info, fh);

  /* pruning downward pass */
  pruning_downward(th, site_id, paths, the_model.init_T, seg_info, fh, gen,
                  proposed_path, FIX_ROOT);
}
