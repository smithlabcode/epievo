...
PatSeq::PatSeq(const std::vector<bool> &seq) {
  ...
  pos_by_pat.resize(1, 0);    
  cum_pat_freq.resize(8, 0);
  for (size_t i = 0; i < 8; ++i) {
    pos_by_pat.insert(pos_by_pat.end(), pos_grp[i].begin(), pos_grp[i].end());
    cum_pat_freq[i] = (i==0)? pos_grp[i].size() : cum_pat_freq[i-1] + pos_grp[i].size();
  }
  pos_by_pat.push_back(0);  

  /* set idx_in_pat */
  idx_in_pat.resize(n, 0);
  for (size_t i = 1; i < pos_by_pat.size() - 1; ++i) {
...
size_t PatSeq::get_context(const size_t pos) const {
  assert(pos > 0 && pos < pos_by_pat.size());
...
void PatSeq::single_update(const size_t pos, const size_t context, const size_t to_context) {
  ...
  if (to_context < context) {
    ...
    while (cur_context > to_context) {
      size_t prev_context = context - 1; 
      size_t cur_block_start = cum_pat_freq[cur_context - 1] + 1;
      size_t prev_block_start = (prev_context > 0)? (cum_pat_freq[cur_context - 1] + 1) : 1;
      iter_swap(pos_by_pat.begin() + cur_block_start, pos_by_pat.begin() + prev_block_start);    
    ...
    size_t cur_context = context;
    while (cur_context < to_context) {
      size_t next_context = context + 1;
      size_t cur_block_end = cum_pat_freq[cur_context];
      size_t next_block_end = cum_pat_freq[next_context];
      iter_swap(pos_by_pat.begin() + cur_block_end, pos_by_pat.begin() + next_block_end);
...
void PatSeq::to_seq(std::vector<bool> seq) const {
  ...
  seq = std::vector<bool>(n_site, true);
  size_t idx = 1;
