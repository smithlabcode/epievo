...
\]
This is an \textit{independence sampler}. Suppose the current guess of path at position $n$ is $L_n$, we accept the move to the proposed path $L'$ with probability
...
\paragraph{Posterior distribution of a path given two neighboring
paths in a tree structure with fixed leaf data} Suppose we are given a
...
chain conditional on end-point states.  
