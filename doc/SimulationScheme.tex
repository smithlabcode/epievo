...
\begin{equation}
  \begin{aligned}
    ...
    = ~ & \sum\limits_{c=0,2,5}\bigg(J_c\log\lambda_c - D_{c}\lambda_c\bigg) ~ + 
    ~ \sum\limits_{c=1,3} \bigg((J_c + J_{c'})\log\lambda_c ~ - 
    (D_{c} + D_{c'})\lambda_c\bigg) ~ + \\
    ~ & \bigg(J_7\log(\frac{\lambda_0\lambda_3^2\lambda_5}{\lambda_2\lambda_1^2})
    - D_{7}\cdot\frac{\lambda_0\lambda_3^2\lambda_5}{\lambda_2\lambda_1^2} \bigg)
  \end{aligned}
\end{equation}
where $J_c = \sum\limits_{b=1}^B\sum\limits_{m=1}^{M_b-1}I_{\{c_{b,m}
= c\}}$, and $D_{c} = \sum\limits_{b=1}^B\sum\limits_{m=1}^{M_b}I_{\{c_{b,m} =
c\}}(t_{b,m} - t_{b,m-1})$.
