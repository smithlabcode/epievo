...
root <- read.table(sprintf("%s.stats.root", filepath.true), header=T, sep="\t")
root2 <- read.table(sprintf("%s.stats.root", filepath.est), header=T, sep="\t")
...
# root
for (j in 1:ncol(root)) {
  hist(root[, j], col=rgb(1,0,0,0.5), freq=T, xlim=c(-1, 1), breaks=-1:1,
       main=paste0("ROOT SITE: ", j), font.main = 1,
       xlab="ROOT", ylim=c(0, histlim))
  hist(root2[, j], col=rgb(0,0,1,0.5), freq=T, xlim=c(-1, 1), breaks=-1:1,
       ylim=c(0, histlim), add=T)
  legend("topright", c("true", "sampled"),
